/*
 * drivers/cpufreq/cpufreq_mazwoz.c
 *
 ...
 *
 * For a general overview of mazwoz see the relavent part in
...
static DEFINE_PER_CPU(struct mazwoz_info_s, mazwoz_info);
...
static int cpufreq_governor_mazwoz(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_MAZWOZ
static
#endif
struct cpufreq_governor cpufreq_gov_mazwoz = {
	.name = "mazwoz",
	.governor = cpufreq_governor_mazwoz,
	.max_transition_latency = 9000000,
	.owner = THIS_MODULE,
};

inline static void mazwoz_update_min_max(struct mazwoz_info_s *this_mazwoz, struct cpufreq_policy *policy, int suspend) {
	if (suspend) {
		this_mazwoz->ideal_speed = // sleep_ideal_freq; but make sure it obeys the policy min/max
			policy->max > sleep_ideal_freq ?
			(sleep_ideal_freq > policy->min ? sleep_ideal_freq : policy->min) : policy->max;
	} else {
		this_mazwoz->ideal_speed = // awake_ideal_freq; but make sure it obeys the policy min/max
			policy->min < awake_ideal_freq ?
			(awake_ideal_freq < policy->max ? awake_ideal_freq : policy->max) : policy->min;
	}
}

inline static void mazwoz_update_min_max_allcpus(void) {
	unsigned int i;
	for_each_online_cpu(i) {
		struct mazwoz_info_s *this_mazwoz = &per_cpu(mazwoz_info, i);
		if (this_mazwoz->enable)
			mazwoz_update_min_max(this_mazwoz,this_mazwoz->cur_policy,suspended);
	}
}
...
inline static void reset_timer(unsigned long cpu, struct mazwoz_info_s *this_mazwoz) {
	this_mazwoz->time_in_idle = get_cpu_idle_time_us(cpu, &this_mazwoz->idle_exit_time);
	mod_timer(&this_mazwoz->timer, jiffies + sample_rate_jiffies);
}
...
inline static int target_freq(struct cpufreq_policy *policy, struct mazwoz_info_s *this_mazwoz,
			      int new_freq, int old_freq, int prefered_relation) {
	int index, target;
	struct cpufreq_frequency_table *table = this_mazwoz->freq_table;
	    ...
	    !cpufreq_frequency_table_target(policy,table,new_freq,prefered_relation,&index))
	{
		...
		if (target == old_freq) {
			...
			printk(KERN_WARNING "mazwoz: frequency change failed: %d to %d => %d\n",
	...
	dprintk(mazwoz_DEBUG_JUMPS,"mazwozQ: jumping from %d to %d => %d (%d)\n",
		old_freq,new_freq,target,policy->cur);

	return target;
}

static void cpufreq_mazwoz_timer(unsigned long cpu)
{
	u64 delta_idle;
	u64 delta_time;
	int cpu_load;
	...
	u64 now_idle;
	int queued_work = 0;
	struct mazwoz_info_s *this_mazwoz = &per_cpu(mazwoz_info, cpu);
	struct cpufreq_policy *policy = this_mazwoz->cur_policy;

	now_idle = get_cpu_idle_time_us(cpu, &update_time);
	old_freq = policy->cur;

	if (this_mazwoz->idle_exit_time == 0 || update_time == this_mazwoz->idle_exit_time)
		return;

	delta_idle = cputime64_sub(now_idle, this_mazwoz->time_in_idle);
	delta_time = cputime64_sub(update_time, this_mazwoz->idle_exit_time);

	// If timer ran less than 1ms after short-term sample started, retry.
	if (delta_time < 1000) {
		if (!timer_pending(&this_mazwoz->timer))
			reset_timer(cpu,this_mazwoz);
		return;
	}
	...
	dprintk(mazwoz_DEBUG_LOAD,"mazwozT @ %d: load %d (delta_time %llu)\n",
		old_freq,cpu_load,delta_time);

	this_mazwoz->cur_cpu_load = cpu_load;
	this_mazwoz->old_freq = old_freq;
	...
	if (cpu_load > max_cpu_load || delta_idle == 0)
	{
		if (old_freq < policy->max &&
			 (old_freq < this_mazwoz->ideal_speed || delta_idle == 0 ||
			  cputime64_sub(update_time, this_mazwoz->freq_change_time) >= up_rate_us))
		{
			dprintk(mazwoz_DEBUG_ALG,"mazwozT @ %d ramp up: load %d (delta_idle %llu)\n",
				old_freq,cpu_load,delta_idle);
			this_mazwoz->ramp_dir = 1;
			work_cpumask_set(cpu);
			queue_work(up_wq, &freq_scale_work);
			queued_work = 1;
		}
		else this_mazwoz->ramp_dir = 0;
	}
	...
	// frequency we require that we have been at this frequency for at least down_rate_us:
		 ...
		 (old_freq > this_mazwoz->ideal_speed ||
		  cputime64_sub(update_time, this_mazwoz->freq_change_time) >= down_rate_us))
	{
		dprintk(mazwoz_DEBUG_ALG,"mazwozT @ %d ramp down: load %d (delta_idle %llu)\n",
			old_freq,cpu_load,delta_idle);
		this_mazwoz->ramp_dir = -1;
		work_cpumask_set(cpu);
...
{
	struct mazwoz_info_s *this_mazwoz = &per_cpu(mazwoz_info, smp_processor_id());
	struct cpufreq_policy *policy = this_mazwoz->cur_policy;
	...
	if (policy->cur == policy->min && timer_pending(&this_mazwoz->timer))
		del_timer(&this_mazwoz->timer);

	pm_idle_old();

	if (!timer_pending(&this_mazwoz->timer))
		reset_timer(smp_processor_id(), this_mazwoz);
}

/* We use the same work function to sale up and down */
static void cpufreq_mazwoz_freq_change_time_work(struct work_struct *work)
{
	unsigned int cpu;
	int new_freq;
	int old_freq;
	int ramp_dir;
	struct mazwoz_info_s *this_mazwoz;
	struct cpufreq_policy *policy;
	unsigned int relation = CPUFREQ_RELATION_L;
	for_each_possible_cpu(cpu) {
		this_mazwoz = &per_cpu(mazwoz_info, cpu);
		if (!work_cpumask_test_and_clear(cpu))
			continue;

		ramp_dir = this_mazwoz->ramp_dir;
		this_mazwoz->ramp_dir = 0;

		old_freq = this_mazwoz->old_freq;
		policy = this_mazwoz->cur_policy;

		if (old_freq != policy->cur) {
			// frequency was changed by someone else?
			printk(KERN_WARNING "mazwoz: frequency changed by 3rd party: %d to %d\n",
			       old_freq,policy->cur);
			new_freq = old_freq;
		}
		else if (ramp_dir > 0 && nr_running() > 1) {
			// ramp up logic:
			if (old_freq < this_mazwoz->ideal_speed)
				new_freq = this_mazwoz->ideal_speed;
			...
			dprintk(mazwoz_DEBUG_ALG,"mazwozQ @ %d ramp up: ramp_dir=%d ideal=%d\n",
				old_freq,ramp_dir,this_mazwoz->ideal_speed);
		}
		else if (ramp_dir < 0) {
			// ramp down logic:
			if (old_freq > this_mazwoz->ideal_speed) {
				...
				new_freq = old_freq * this_mazwoz->cur_cpu_load / max_cpu_load;
				if (new_freq > old_freq) // min_cpu_load > max_cpu_load ?!
					new_freq = old_freq -1;
			}
			dprintk(mazwoz_DEBUG_ALG,"mazwozQ @ %d ramp down: ramp_dir=%d ideal=%d\n",
				old_freq,ramp_dir,this_mazwoz->ideal_speed);
		}
			...
			new_freq = old_freq;
			dprintk(mazwoz_DEBUG_ALG,"mazwozQ @ %d nothing: ramp_dir=%d nr_running=%lu\n",
				old_freq,ramp_dir,nr_running());
		}

		// do actual ramp up (returns 0, if frequency change failed):
		new_freq = target_freq(policy,this_mazwoz,new_freq,old_freq,relation);
		if (new_freq)
			this_mazwoz->freq_change_time_in_idle =
				get_cpu_idle_time_us(cpu,&this_mazwoz->freq_change_time);
		...
		else if (timer_pending(&this_mazwoz->timer))
			del_timer(&this_mazwoz->timer);
	}
}
...
static struct attribute * mazwoz_attributes[] = {
...
static struct attribute_group mazwoz_attr_group = {
	.attrs = mazwoz_attributes,
	.name = "mazwoz",
};

static int cpufreq_governor_mazwoz(struct cpufreq_policy *new_policy,
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_mazwoz->cur_policy = new_policy;

		this_mazwoz->enable = 1;

		mazwoz_update_min_max(this_mazwoz,new_policy,suspended);

		this_mazwoz->freq_table = cpufreq_frequency_get_table(cpu);
		if (!this_mazwoz->freq_table)
			printk(KERN_WARNING "mazwoz: no frequency table for cpu %d?!\n",cpu);
		...
		if (this_mazwoz->cur_policy->cur < new_policy->max && !timer_pending(&this_mazwoz->timer))
			reset_timer(cpu,this_mazwoz);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (this_mazwoz->cur_policy->cur > new_policy->max) {
			dprintk(mazwoz_DEBUG_JUMPS,"mazwozI: jumping to new max freq: %d\n",new_policy->max);
			__cpufreq_driver_target(this_mazwoz->cur_policy,
						new_policy->max, CPUFREQ_RELATION_H);
		}
		else if (this_mazwoz->cur_policy->cur < new_policy->min) {
			dprintk(mazwoz_DEBUG_JUMPS,"mazwozI: jumping to new min freq: %d\n",new_policy->min);
			__cpufreq_driver_target(this_mazwoz->cur_policy,
	...
	case CPUFREQ_GOV_STOP:
		this_mazwoz->enable = 0;
		smp_wmb();
		del_timer(&this_mazwoz->timer);
		flush_work(&freq_scale_work);
		this_mazwoz->idle_exit_time = 0;
...
static void mazwoz_suspend(int cpu, int suspend)
{
	struct mazwoz_info_s *this_mazwoz = &per_cpu(mazwoz_info, smp_processor_id());
	struct cpufreq_policy *policy = this_mazwoz->cur_policy;
	...
	mazwoz_update_min_max(this_mazwoz,policy,suspend);
	if (!suspend) { // resume at max speed:
		new_freq = validate_freq(policy,sleep_wakeup_freq);

		dprintk(mazwoz_DEBUG_JUMPS,"mazwozS: awaking at %d\n",new_freq);
		...
		dprintk(mazwoz_DEBUG_JUMPS,"mazwozS: suspending at %d\n",policy->cur);
	}

	reset_timer(smp_processor_id(),this_mazwoz);
}

static void mazwoz_early_suspend(struct early_suspend *handler) {
	int i;
...
static void mazwoz_late_resume(struct early_suspend *handler) {
	int i;
...
static struct early_suspend mazwoz_power_suspend = {
	.suspend = mazwoz_early_suspend,
	.resume = mazwoz_late_resume,
#ifdef CONFIG_MACH_HERO
	.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 1,
#endif
};

static int __init cpufreq_mazwoz_init(void)
{
	unsigned int i;
	struct mazwoz_info_s *this_mazwoz;
	debug_mask = 0;
	...
	for_each_possible_cpu(i) {
		this_mazwoz = &per_cpu(mazwoz_info, i);
		this_mazwoz->enable = 0;
		this_mazwoz->cur_policy = 0;
		this_mazwoz->ramp_dir = 0;
		this_mazwoz->time_in_idle = 0;
		this_mazwoz->idle_exit_time = 0;
		this_mazwoz->freq_change_time = 0;
		this_mazwoz->freq_change_time_in_idle = 0;
		this_mazwoz->cur_cpu_load = 0;
		// intialize timer:
		init_timer_deferrable(&this_mazwoz->timer);
		this_mazwoz->timer.function = cpufreq_mazwoz_timer;
		this_mazwoz->timer.data = i;
		work_cpumask_test_and_clear(i);
	}

	// Scale up is high priority
	up_wq = alloc_workqueue("kmazwoz_up", WQ_HIGHPRI, 1);
	down_wq = alloc_workqueue("kmazwoz_down", 0, 1);
	if (!up_wq || !down_wq)
		return -ENOMEM;

	INIT_WORK(&freq_scale_work, cpufreq_mazwoz_freq_change_time_work);

	register_early_suspend(&mazwoz_power_suspend);

	return cpufreq_register_governor(&cpufreq_gov_mazwoz);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_MAZWOZ
fs_initcall(cpufreq_mazwoz_init);
#else
module_init(cpufreq_mazwoz_init);
#endif

static void __exit cpufreq_mazwoz_exit(void)
{
	cpufreq_unregister_governor(&cpufreq_gov_mazwoz);
	destroy_workqueue(up_wq);
	destroy_workqueue(down_wq);
}

module_exit(cpufreq_mazwoz_exit);

MODULE_AUTHOR ("MazWoz");
MODULE_DESCRIPTION ("'cpufreq_mazwoz' - A smarter cpufreq governor based off of SMARTASSV2");
MODULE_LICENSE ("GPL");
